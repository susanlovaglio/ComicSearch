...
class ComicVineAPIClient{
    ...
    class func getCharacters(with query: String, offset: Int, with completion: @escaping ([[String: Any]]) -> ()){
        
        let stringUrl = "http://comicvine.com/api/search/?api_key=\(Secrets.key)&format=json&offset=\(offset)&query=\(query)&resources=character&field_list=name,image"
